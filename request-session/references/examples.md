...
402
```
Parsed: the service requires **1.00 USDC** per request for `/v1/forecast`. (`network` is CAIP-2 — `eip155:8453` is base, `eip155:137` is Polygon, `eip155:43114` is Avalanche, `eip155:4663` is Robinhood Chain, and `eip155:5042002` is Arc testnet (dev only); an EVM chain is `eip155:<chainId>`, and a Solana endpoint advertises `solana:<…>`. You only need `asset` and `amount` to build the delegation; the backend settles on the merchant's chain automatically — on mainnet that is base, Polygon, Avalanche, tempo, solana, or Robinhood Chain; the dev environment serves arc only.)
