...
For the money, read the `settlement` legs rather than `seller_bps` alone. The legs are the amounts that actually moved, and a deal where a fee also moved value does not sum from the basis points by themselves. `kagent agreement status` carries `seller_bps` and the legs once the engine has them, so quantifying the split needs no chain query.
