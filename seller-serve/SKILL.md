...
- **Whatever value you land on, it must stay comfortably *below* the buyer's message TTL — this constraint only bites the `request`/`decide` message lane, not `start`/deliverable work.** serve only attempts a `request`/`decide` item whose remaining TTL is strictly greater than `--handler-timeout`; one it could not finish before the message expires is discarded as `moot` before the handler ever runs — no answer is ever produced. Since kagent **v5.1.1** the discard is at least visible: serve logs the moot on its own output, and for a request whose claim is still live it relays a `reply/v1` decline naming the TTL-vs-budget mismatch, so the buyer sees `replied` with a reason instead of a bare `expired`. On a pre-v5.1.1 serve the discard is fully silent on both sides — the buyer just sees the message quietly reach `expired`. Either way the deal doesn't advance: a buyer's default message TTL is 10 minutes (`buyer-purchase`'s `message send`), and if this seller's real deliverable work needs a longer `--handler-timeout` than that, the fix is **not** to shrink it back down — negotiation and acceptance-check messages would still be discarded either way — the fix is telling buyers negotiating with this seller to send request-frame messages with a longer explicit `--ttl` (up to `1h`) that clears whatever `--handler-timeout` this seller actually runs — sent as a fresh message (a new or omitted `--idempotency-key`), since reusing the expired send's key just returns the original message with its old TTL.
...
| A correctly-framed request is claimed but never quoted | Its remaining TTL was not strictly greater than `--handler-timeout` when serve claimed it — discarded as `moot` before the handler ran. Since kagent v5.1.1 serve logs the moot and the buyer receives a `reply/v1` decline saying so; on older serves nothing errors anywhere. Lower `--handler-timeout`, or have the buyer resend with a longer `--ttl` as a fresh message (a new or omitted `--idempotency-key` — a reused key returns the expired original) |
