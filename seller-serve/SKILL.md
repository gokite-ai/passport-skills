...
| A correctly-framed request is claimed but never quoted | Its remaining TTL was not strictly greater than `--handler-timeout` when serve claimed it — discarded as `moot` before the handler ran. Since kagent v5.1.1 serve logs the moot and the buyer receives a `reply/v1` decline saying so; on older serves nothing errors anywhere. Lower `--handler-timeout`, or have the buyer resend with a longer `--ttl` |
