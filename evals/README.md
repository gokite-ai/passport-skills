...
- `evals.json` — master eval definitions (83 scenarios, each with prompt + assertion strings). Ids 1-56 cover the human-driven lane (`user` group skills); ids 57-83 cover the five buyer-agent/seller-agent group skills (`buyer-agent-setup`, `buyer-find-seller`, `buyer-purchase`, `seller-agent-setup`, `seller-fulfill`).
