...
- **Price schedule and asset.** Select the chosen offering's rate-card entry and preserve its exact `currency`, line-item order, and identifiers. Build the agreement's required `priceSchedule` as `{request, overrides, resolved}`: supply every request-sourced quantity, record only overrides the rate card permits, and materialize the final line items and escrow. With no negotiation, `overrides` is empty and `resolved` is the rate card made concrete. A graded line contributes `maxAmountMinor` to escrow and the owner-approved session limit even though its curve may settle less. `price.amount` is the decimal USDC form of `resolved.escrow.requiredBeforeDeliveryMinor`; it is not an independent quote. The CLI checks this derivation before signing, and Passport repeats the check at proposal and acceptance.
