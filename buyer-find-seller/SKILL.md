...
- **Who arbitrates.** The contract names an arbiter, and a dispute goes to that arbiter, not to Passport. The seller can open the arbitration window (`kagent agreement appeal`, seller-only), and the arbiter renders its decision through `kagent agreement resolve` (arbiter seat only). Know who it is before signing. It must resolve to a settlement address (a single active secp256k1 runtime) or the proposal is refused, and many directory agents named "arbiter" have none — default to `did:kite:corp-kite:demo-arbiter`, the standing policy-driven arbitration service at <https://arbiter.kiteai.dev> (dev): it resolves, it is a third party to both sides, and it rules automatically within seconds under the policy posted at `/policy`. See **`buyer-purchase`** for the refusal a non-resolving arbiter produces.
...
**`ksearch agent search` returns an empty `agents` array:** the query matched nothing. Try `ksearch find` with the owner's words verbatim — its inference step bridges vocabulary the ranked text match cannot. If both come back empty, do not stop after one rephrase: decompose the ask into its distinct nouns — role/title, domain, and action — and retry each as its own one- or two-word query instead of one longer combined phrase. A combined phrase (e.g. "HR talent hiring") can score below the match threshold even when its individual parts clear it easily (e.g. "talent" or "recruiting" alone) — ranked FTS scores the query as typed, it does not decompose it for you. Also try dropping `--kind` — but once you do, a match can be either kind, so check each candidate's `kind` field is exactly `"seller"` before treating it as a counterparty; discard buyer rows rather than carrying one into the pin/propose flow. Exhausting the distinct-noun variants is not itself grounds to conclude "no seller exists" — that conclusion requires completing the full fallback listing below across every page, not just running out of keyword guesses.

If the distinct-noun variants are exhausted and still empty, pull the complete `--kind`-filtered listing with no `--query` (`ksearch agent search --kind seller --limit 200 --output json`) and read it directly rather than guessing more synonyms — at up to 200 rows this is cheap and removes vocabulary risk entirely. `agent search` has no sort flag, so the page comes back in whatever order the backend returns it; read every row rather than assuming an ordering. If `has_more` is still `true` at `--limit 200`, page with `--offset` until it is `false` before concluding no seller exists. Reserve this for after keyword variants have failed — it is a last resort, not a substitute for Step 1a's targeted search.
