...
3. **Active spending session** — Required ONLY for `shop:checkout`. All other commands (search, cart, shipping) do NOT require a session. Before checkout, create a session with budget derived from the cart total. Sessions are protocol-agnostic — any active session with a sufficient asset allowlist and budget cap can be used for checkout; you do not need to mark the session as shopping-specific. See the `shop:checkout` entry in `@references/commands.md` for the Pre-Checkout Checklist.
