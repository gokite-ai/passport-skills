...
Local pre-flight. It checks what this machine can check: JSON validity, the schema identifiers, `agentDid` agreement across the three files, offering and item ids (grammar, uniqueness — duplicates in either file are caught), BOTH coverage set-equalities (rate card = storefront = workflow/terms), the `fixed/v1`/`negotiated/v1` model unions (amounts required on fixed, forbidden on negotiated), per-kind line rules (stray members refused — a flat line carrying `unitPriceMinor` fails; unit kinds `count|time|volume` with `month|hour|minute` time units; a metered line declares its escrow; a graded curve's scores strictly increase and no payout exceeds `maxAmountMinor`), the payout union, negotiable entries (each present bound held to the money grammar on its own, the `field` matched to the line's actual price field, min ≤ max, and **no `(itemId, field)` pair repeated** — a repeat is refused naming the earlier index: `itemId <id> field <field> is already declared negotiable at index <n>`, because the formation reader keys these into a map and a repeat would leave the enforced bound decided by array order rather than by the published document), the money grammar (integer strings in minor units, at most 30 digits), and the **worked-example identity** — funding recomputed line by line must equal the example exactly, the same proof the platform enforces. A storefront still carrying `price`/`settlement` is told those members moved to the rate card. Workflow template ids are resolved against the public template registry unless `--offline`; for a workflow/terms **v1** document (`…schema:workflow-terms:v1`, per-offering `workflow.{templateId, config}`), each unique `{templateId, config}` pair is additionally dry-run against the platform validator (`POST /v1/agent/workflows:validate` — the exact validation and hashing publish runs, creating nothing). The dry-run needs the runtime key; without one it is reported as skipped, never invented. The config's one structural rule is that it is a JSON object — the platform records it verbatim and does not interpret it. The skeleton emits a bare `"config": {}` (the template's pure defaults); to override, add members following the **configuration format v0** convention:
...
**An override member left EMPTY — `"windows": {}`, `"limits": {}`, `"disabledCommands": []`, `"markdown": {}` — is not equivalent to omitting it, and has been observed to fail the platform dry-run for at least the `standard/v1` template** (`registration validate` exit 8, `"the platform dry-run refused this workflow: Request failed (HTTP 422): workflow refused"`, pointing at `/offerings/<n>/workflow`), even though the schema itself treats all members as optional. Two known-safe paths: fill each member with real values you actually want, or delete every empty member down to a bare `"config": {}`. Since kpass/kagent **v5.1.1** the skeleton emits `"config": {}` (older skeletons emitted the empty-member form and failed unedited), and a dry-run refusal names any empty override members it finds in the refused config — so if you see this refusal on a hand-written config, the empty members are the first thing to delete.
