...
- `kagent workflows` / `workflow show <id>` — the TEMPLATE group is `workflow-template` with children `list` and `get <family/version>`; `workflow-template list` is where `registration template`'s `<template id>` placeholder gets its value. On kagent, bare `workflow` is a DIFFERENT group: this seller's own immutable Workflows — `kagent workflow list` (signed management view) and `kagent workflow get <workflow-hash>` (one Workflow, complete config included).
