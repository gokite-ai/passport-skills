...
```bash
kpass agent token create --agent did:kite:ind-alice:my-buyer --output json
```
...
```json
{
  ...
  "hint": "Use this once: kpass agent bind --agent did:kite:ind-alice:my-buyer --token art_... --output json (or kagent bind for a seller runtime). It will not be shown again."
}
```

The plaintext `token` is shown exactly once — the server stores only its hash. Consume it immediately with `kpass agent bind --token <art_...>`.
