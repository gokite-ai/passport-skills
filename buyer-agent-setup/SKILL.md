...
Once this buyer agent is bound, **`kpass agent session request` (see the `buyer-purchase` skill) is the ONLY way to request a spending session for it.** The human lane's `kpass session create` (formerly `kpass agent session create`, now a tombstone) is a different lane -- the human's JWT authorizing an agent to spend on paid APIs and shopping -- and it cannot express the v2 scope an agreement needs, so a session from it is refused at funding with `session_scope_forbidden`. Route on what the money pays for: an agreement means `kpass agent session request`, a priced API call or a checkout means `kpass session create`.
