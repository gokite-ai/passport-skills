...
```bash
kpass session attach --session-id <SESSION_ID> --output json
```
...
```json
{
  ...
  "next_command": "kpass session status --request-id req_abc123 --wait --output json"
}
```
...
CLI sets it as the current session automatically (no `session use`
...
- Attach is once-only. If the session is already attached to *this* agent,
  it may simply be usable — check `kpass session list --output json`
  and set it current with `session use`. If it is attached to a
...
- `kpass session detach` — attach is permanent; detach was removed
  from the CLI
- `kpass session attach <session_id>` (positional) — the session ID
  must be passed via `--session-id`
- `kpass session attach --public-key …` — the keypair is generated by
  the CLI automatically; there is no key flag
- `kpass session attach --request-id …` — `--request-id` belongs to
  `session status`, not `attach`
- `kpass session status --session-id` — the flag is `--request-id`
- Any command with `--json` — the correct flag is `--output json` (two
  separate tokens)

Also out of scope for this skill: `session create --type attachable`
...
- **No session yet / needs a new session:** **`request-session`** discovers
  ...
  full `agent:register` and `session status` references this skill
  relies on.
