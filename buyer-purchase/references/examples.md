...
The terms file carries the business half of the contract only. The CLI owns `schema`, `buyerAgentId`, `sellerAgentId`, `runtimeBinding`, `signatures`, `termsHash` — and `workflowId` — including any of them is exit 2.
...
**`workflowId` is absent on purpose — do not add it.** The seller declares one
workflow per offering in its registration; `propose` reads it from the offering
`registrationBasis.offeringId` names and writes it in. A terms file naming a
different one is refused before anything is signed, and there is no flag for it:
