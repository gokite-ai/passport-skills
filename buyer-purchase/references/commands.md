...
What the command does, in order: reads the seller's active registration and takes the `workflowId` — plus, when the offering row carries its current workflow binding, the `workflowHash` and `workflowBindingRevision` pair — from the offering `registrationBasis.offeringId` names (a terms file naming a different id or hash is refused here, before anything is signed), derives the contract (pinning that workflow, the schema, both agent ids, and a `runtimeBinding` of `{runtimeAgentId, agentCardHash, extensionUri, endpoint}` from the pin), validates it against the vendored schema, canonicalizes it, signs the terms hash, re-validates, re-derives the hash and asserts it did not move, journals the proposal **before sending**, sends it, and then relays the formation co-signature.
