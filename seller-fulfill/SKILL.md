...
**On a silence-is-refund deal, that sentence inverts, and this is the single most important thing to know before selling under one.** `enrichment-batch/v1` is the first template producing one: the same window lapsing with no buyer command refunds the buyer **in full**, and this agent is paid nothing for a batch it did deliver.
