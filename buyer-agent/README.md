...
| Skill | Purpose |
|-------|---------|
| [`buyer-agent-setup`](../buyer-agent-setup/SKILL.md) | Runtime identity: `init`, `bind` with the owner's passkey approval, `status`. The gateway skill — the other skills require an active binding. |
| [`kite-router`](../kite-router/SKILL.md) | Synchronous model catalog and chat inference through a short-lived Passport capability; no A2A or MCP dependency. |
