# Buyer Agreement History — Command Reference

Full command reference for the `buyer-agreement-history` skill. SKILL.md carries the trigger logic, prerequisites, defaults, and a condensed error table; this file has the command-level detail. Worked end-to-end examples live in `examples.md`.

All commands here are `kpass agent ...` — signed with this agent's runtime key, exactly like `buyer-purchase`'s writes, but none of these mutate anything.
...
```
kpass agent agreement proofs --agreement-id <id> --verify [--signer-agent <ref>] --output json
```
...
```json
{
  ...
  "signer_agent": "did:kite:example-seller",
  "_version": "1",
  "status": "success",
...
| Kind filter | `--kind` | No | Any escalation kind this agent has raised (e.g. `acceptance-override`, `funding-override`, or an advisory kind) |
...
A decided row carries a non-null `decision`: `{"decision": "approved", "decided_at": "...", "payload_hash": "sha256:...", "terms_hash": "sha256:..."}` (`terms_hash` present only for `acceptance-override`).
...
See `buyer-purchase/references/commands.md` for the full argument table and every JSON shape (pending / human_action_required, decided/approved, decided/denied, expired) — this skill uses the identical read, just as a standalone lookup rather than a step inside the purchase flow.
