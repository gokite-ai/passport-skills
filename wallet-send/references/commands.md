...
- `assets[]` — one entry per asset symbol.
  ...
  - `chains[]` — per-chain breakdown: `{ chain, amount, partial }`. `chain` is `base`, `polygon`, `tempo`, `solana`, or `robinhood`.
...
```
kpass wallet send --chain <base|polygon|tempo|solana|robinhood> --to <RECIPIENT_ADDRESS> --amount <N> --asset <SYMBOL> --output json
```
...
| Argument | Flag | Required | Source | Validation |
|----------|------|----------|--------|------------|
| Chain | `--chain` | **Yes** | Ask the user | One of `base`, `polygon`, `tempo`, `solana`, `robinhood`. No default. `kite` and anything else are rejected (exit 2). |
| Recipient address | `--to` | Yes | Ask the user | Validated **for the chosen chain**: base/polygon/tempo/robinhood = EVM `0x` + 40 hex (EIP-55 checksum enforced when mixed-case); solana = base58 decoding to 32 bytes. Invalid → exit 2 before any network call. |
| Amount | `--amount` | Yes | Ask the user | Positive decimal string (e.g. `"25"`, `"0.50"`). |
| Asset symbol | `--asset` | Yes | Ask the user | Token symbol: `USDC` on base/polygon/tempo/solana, `PYUSD` on solana, `USDG` on robinhood. |
...
```json
{ "_version": "1", "status": "error", "error": "Missing --chain flag. Usage: kpass wallet send --chain <base|polygon|tempo|solana|robinhood> --to <RECIPIENT_ADDRESS> --amount <N> --asset <SYMBOL> --output json", "hint": "", "next_command": "" }
```

Other exit-2 messages: `--chain must be one of base|polygon|tempo|solana|robinhood (got: "...")`; `--to is not a valid <chain> address: EVM address must be 0x + 40 hex chars (got N)` / `EVM address fails EIP-55 checksum (possible typo)` / `Solana address must decode to 32 bytes (got N)`; `--amount must be a positive number (got: "...")`; `Missing --to flag`; `Missing --asset flag`.
...
| Argument | Flag | Required | Source | Validation |
|----------|------|----------|--------|------------|
| Chain filter | `--chain` | No | Pass to show one chain | One of `base`, `polygon`, `tempo`, `solana`, `robinhood`. Omit for all. Invalid → exit 2. |
| Output format | `--output json` | Yes | Always pass | Literal value `json` |
...
- `wallets[]` — `{ chain, vm_family, address }`.
  - `vm_family` is `"evm"` for base/polygon/tempo/robinhood and `"solana"` for solana.
  - **base, polygon, tempo, and robinhood share one EVM address** (same `address` value). When rows match, tell the user it is one wallet, not a duplicate.
