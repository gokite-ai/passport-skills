...
Picking the row is only half of deal shape. The **Windows** column tells you which deadline windows the seller must then set (phase 3b in the SKILL): a template is a *shape*; its window durations and `maxRedeliveries` are per-offering config the seller chooses, recorded verbatim in the offering's `workflow.config`. **The windows a template has must be non-zero for a deal to activate** -- never publish an empty config for a template that has windows. Ask about `maxRedeliveries` only for the mid group (`coding`/`content-generator`/`security-audit`); `standard/v1` and the no-reject templates have no redelivery lane, so don't. Only `coding/v1` supports buyer-initiated `amend`.
