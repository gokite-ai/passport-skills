...
- **Structural** (Windows, Max redeliveries, Escalation path) -- also verified directly against the descriptor JSON (re-verified 2026-09-02 against `pkg/a2a/templates/v1/*.json` on `origin/main`). Two corrections from that pass: `coding/v1` is the **only** template with an `amend` transition (do not describe it as "same shape as `content-generator`"); and **Max redeliveries applies only where a `REDELIVERING` state exists** -- the mid group (`coding`/`content-generator`/`security-audit`). The config schema carries a `maxRedeliveries` field for every template, but it is behaviorally inapplicable to `standard/v1` (dispute lane instead) and the no-reject templates. Re-read the descriptor JSON if this table is ever suspected stale.
...
|---|---|---|---|---|---|---|
| `standard/v1` | Standard delivery | The full agreement lifecycle with delivery review, appeal, and third-party arbitration. | funding, delivery, deliveryConfirmation, appeal, arbitration | -- (no redelivery lane) | Full: reject -> appeal -> dispute -> arbitration -> resolve | Default choice for anything where a buyer might reasonably dispute quality and you want a formal appeal/arbitration escalation available as a last resort. |
| `recruiting/v1` | Recruiting | Candidate-sourcing delivery with buyer confirmation and no dispute branch. | funding, delivery, deliveryConfirmation | -- (no reject lane) | None: delivered or not; only `REFUNDING_UNDELIVERED` available | Candidate-sourcing / matching work where "delivered" is binary (a candidate list either arrived or didn't) -- no post-delivery quality dispute lane. |
| `data-seller/v1` | Data seller | Dataset delivery with buyer confirmation and no dispute branch. | funding, delivery, deliveryConfirmation | -- (no reject lane) | None: same shape as `recruiting/v1` | Data/dataset delivery where the artifact is verifiable at delivery time (hash match) and there's no meaningful "I don't like the data" rejection path. |
...
| `coding/v1` | Coding | Software-deliverable workflow with rejection and bounded redelivery, no arbitration. | funding, delivery, deliveryConfirmation, appeal | 0-3 | Mid + amend: reject -> redeliver -> refund, **plus** a buyer-initiated `amend` (revision) transition -- **the only template with `amend`** | Code/implementation deliverables -- redeliver-on-reject fits "the tests didn't pass, try again", and `amend` lets a buyer request an in-flight revision without a full reject. |
| `security-audit/v1` | Security audit | Audit-report delivery with rejection and bounded redelivery, no arbitration. | funding, delivery, deliveryConfirmation, appeal | 0-3 | Mid: reject -> redeliver -> refund; no appeal/dispute/arbitration escalation | Audit/report deliverables with a possible one-shot revision, no formal appeal process. |

> Note on `appealResponseWindow` (the mid group -- `coding`/`content-generator`/`security-audit`): the descriptor names this window `appealResponseWindow`, but on these three it is the seller's **redelivery-response deadline** (redo or consent-refund after a reject), **not** a formal buyer appeal -- their Escalation column is "no appeal/dispute/arbitration escalation" for exactly that reason. Only on `standard/v1` is `appealResponseWindow` an actual appeal-into-arbitration window. When phase 3b defaults this window, describe it by its real role for the chosen template.
...
## Then configure it (phase 3b)

Picking the row is only half of deal shape. The **Windows** column tells you which deadline windows the seller *can* set (phase 3b in the SKILL): a template is a *shape*; its window durations and `maxRedeliveries` are per-offering config the seller chooses, recorded in the offering's `workflow.config`. **A window the seller doesn't set inherits the template's descriptor default, so an empty or partial `config` is valid and still activates** -- but never emit an explicit `0` (it replaces the default and is rejected, min 45s). Ask about `maxRedeliveries` only for the mid group (`coding`/`content-generator`/`security-audit`); `standard/v1` and the no-reject templates have no redelivery lane, so don't. Only `coding/v1` supports buyer-initiated `amend`.
