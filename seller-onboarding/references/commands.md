...
The workflow-terms input carries, per offering, a `workflow` member `{ "templateId": "<id>", "config": <phase-3b config> }`. `config` is recorded verbatim, content-addressed, not interpreted by the platform (`seller-agent-setup/references/commands.md`, workflow-terms v1 skeleton). Publish the phase-3b `config`; any window the seller left unset inherits the template's descriptor default (an empty or partial `config` is valid), and an explicit `0` is rejected (min 45s). Dry-run a `{templateId, config}` pair with `POST /v1/agent/workflows:validate` (seller runtime key): it runs the same validation + hashing a publish would and returns the **`resolvedConfig`** (authored merged over defaults) -- show the seller that resolved set so phase 3 matches what phase 6 ships. Reconfiguring an offering's workflow later is a **registration republish** (atomic full-replace) -- there is no per-offering PUT.
...
2. The mandate <-> standing-orders floor is the one link no platform call verifies. Read the mandate back (`curl -H "Authorization: Bearer <owner-jwt>" .../acceptancePolicy`, the phase-4 GET) and compare against the floor value the skill already holds (do **not** re-read the seller-acceptance file):
   - **Negotiated (reserve set):** `price_floors[<template>]` must equal the reserve floor and be `<=` the advertised card price.
   - **Fixed-price (no reserve):** there is **no** `price_floors` entry (correct); confirm the standing-orders floor equals the advertised card price.
   On a mismatch (usually a later dashboard edit), **surface it to the owner and let the owner confirm the corrected value -- do not silently re-derive or pick one** -- then update and republish. This is the replacement for a standalone check script: the platform validates what it can, and this readback covers the gap it can't.
