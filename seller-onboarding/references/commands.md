...
**Phase 3b (configuration)** dry-runs the config against the template to resolve its defaults, then computes `workflow.config`. Dry-run (owner JWT -- the same credential phase 4 uses): `POST $KITE_PASSPORT_BASE_URL/v1/agents/<agent>/workflows:validate`, body `{"workflow":{"templateId":"<id>","config":<authored-or-{}>}}` -> `data.resolvedConfig` (authored merged over the descriptor defaults; `Valid`, `workflowHash`, `configHash` also returned). This is the only way to show the seller the real deadlines -- `ksearch workflow-template get` and `kagent registration validate` do not surface the resolved config. The configurable surface (descriptor `configuration.schema`, verified against `pkg/a2a/templates/v1/*.json`):
